...
export function FeeTable({ fees, paidAmounts, onAmountChange, alreadyPaidAmounts, isFieldDisabled }) {
  ...
  const calculateCombinedAmounts = () => {
    const combined = {
      inscription: (alreadyPaidAmounts?.inscription || 0) + (parseFloat(paidAmounts.inscription) || 0),
      fraisAnnexes: (alreadyPaidAmounts?.fraisAnnexes || 0) + (parseFloat(paidAmounts.fraisAnnexes) || 0), // AJOUT
      pension_0: (alreadyPaidAmounts?.pension_0 || 0) + (parseFloat(paidAmounts.pension_0) || 0),
      pension_1: (alreadyPaidAmounts?.pension_1 || 0) + (parseFloat(paidAmounts.pension_1) || 0),
      pension_2: (alreadyPaidAmounts?.pension_2 || 0) + (parseFloat(paidAmounts.pension_2) || 0)
    };
    ...
    <div className="fee-table-container">
      ...
      <div className="table-wrapper">
        <table className="fee-table">
          ...
          <tbody className="table-body">
            ...
            {/* Ligne Frais annexes - AJOUT */}
            <tr className="table-row">
              <td className="table-cell tariff-cell">Frais annexes de scolarité</td>
              <td className="table-cell amount-cell">{formatCurrency(fees.fraisAnnexes || 0)}</td>
              <td className="table-cell paid-cell">
                {formatCurrency(combinedAmounts.fraisAnnexes)}
              </td>
              <td className="table-cell remaining-cell">
                {formatCurrency(Math.max(0, (fees.fraisAnnexes || 0) - combinedAmounts.fraisAnnexes))}
              </td>
            </tr>
      ...
      <div className="new-payment-section">
        ...
        <div className="payment-inputs-grid">
          ...
          {/* Input pour les frais annexes - AJOUT */}
          <div className="payment-input-group">
            <label className="payment-label">Frais annexes</label>
            <input
              type="number"
              value={paidAmounts.fraisAnnexes || ''}
              onChange={(e) => onAmountChange('fraisAnnexes', e.target.value)}
              className="payment-input"
              placeholder="0"
              min="0"
              max={(fees.fraisAnnexes || 0) - (alreadyPaidAmounts?.fraisAnnexes || 0)}
              disabled={isFieldDisabled && isFieldDisabled('fraisAnnexes')}
            />
            <span className="payment-max">
              Max: {formatCurrency((fees.fraisAnnexes || 0) - (alreadyPaidAmounts?.fraisAnnexes || 0))}
            </span>
          </div>
